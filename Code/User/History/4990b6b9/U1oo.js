...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const pywalToggle = document.getElementById("pywalToggle");
    
        // Load the saved state of the Pywal toggle
        const pywalEnabled = localStorage.getItem("pywalEnabled") === "true";
        pywalToggle.checked = pywalEnabled;
    
        if (pywalEnabled) {
            applyPywalTheme();
        }
    
        pywalToggle.addEventListener("change", function () {
            if (pywalToggle.checked) {
                applyPywalTheme();
                localStorage.setItem("pywalEnabled", "true");
            } else {
                resetTheme();
                localStorage.setItem("pywalEnabled", "false");
            }
        });
    });
    
    function applyPywalTheme() {
        fetch("file:///home/kaushik/.cache/wal/colors.json")
            .then(response => response.json())
            .then(colors => {
                document.documentElement.style.setProperty("--bg-color-blue", colors.color0);
                document.documentElement.style.setProperty("--accentLightTint-blue", colors.color1);
                document.documentElement.style.setProperty("--darkerColor-blue", colors.color2);
                document.documentElement.style.setProperty("--darkColor-blue", colors.color3);
                document.documentElement.style.setProperty("--textColorDark-blue", colors.color4);
                document.documentElement.style.setProperty("--whitishColor-blue", colors.color5);
                changeFaviconColor();
                ApplyLoadingColor();
            })
            .catch(error => console.error('Error fetching Pywal colors:', error));
    }
    
    function resetTheme() {
        // Reset to default theme or previously selected theme
...
window.addEventListener("load", function () {
    if (storedTheme) {
        ...
        applySelectedPywalTheme();
    }
});

function applySelectedPywalTheme() {
    const pywalEnabled = localStorage.getItem("pywalEnabled") === "true";
    if (pywalEnabled) {
        applyPywalTheme();
    }
