...
const colorsPath = `file://${process.env.HOME}/.cache/wal/colors.css`;
const link = document.createElement("link");
link.rel = "stylesheet";
link.href = colorsPath;
...
const applySelectedTheme = (colorValue) => {
    if (colorValue !== "dark" && colorValue !== "pywal") {
        resetDarkTheme();
    ...
    } else if (colorValue === "pywal") {
        document.documentElement.style.setProperty("--bg-color-blue", `var(--bg-color-pywal)`);
        document.documentElement.style.setProperty("--accentLightTint-blue", `var(--accentLightTint-pywal)`);
        document.documentElement.style.setProperty("--darkerColor-blue", `var(--darkerColor-pywal)`);
        document.documentElement.style.setProperty("--darkColor-blue", `var(--darkColor-pywal)`);
        document.documentElement.style.setProperty("--textColorDark-blue", `var(--textColorDark-pywal)`);
        document.documentElement.style.setProperty("--whitishColor-blue", `var(--whitishColor-pywal)`);
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    pywalToggle.addEventListener("change", function () {
        if (this.checked) {
            applySelectedTheme("pywal");
            localStorage.setItem(themeStorageKey, "pywal");
            radioButtons.forEach(radio => {
                radio.checked = false;
            });
        } else {
    ...
    // Load the stored theme on page reload
    const storedTheme = localStorage.getItem(themeStorageKey);
    if (storedTheme === "pywal") {
        pywalToggle.checked = true;
        applySelectedTheme("pywal");
    }
});
